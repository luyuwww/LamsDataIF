...
public class PaginationInterceptor implements Interceptor {
	public Object intercept(Invocation invocation) throws Throwable {
		MappedStatement mappedStatement = (MappedStatement) invocation
				.getArgs()[0];
		if (mappedStatement.getId().contains(pageFilter)) {
			...
			if (page != null && page.isPagination() == true) {
				...
				if (!StringUtils.isBlank(dialect)) {
					...
					if (dialect.equals("mysql")) {
						pagesql = new MySql5Dialect().getLimitString(
								originalSql, rowBounds.getOffset(),
								rowBounds.getLimit());
					} else if (dialect.equals("oracle")) {
						pagesql = new OracleDialect().getLimitString(
								originalSql, rowBounds.getOffset(),
								rowBounds.getLimit());
					} else {
						pagesql = new SqlServerDialect().getLimitString(
								originalSql, rowBounds.getOffset(),
								rowBounds.getLimit(), pagesql, pagesql);
					}
	...
	private static String pageFilter = ""; // mapper.xml中需要拦截的ID(正则匹配)

	public static void setDialect(String dialect) {
		PaginationInterceptor.dialect = dialect;
	}

	public static void setPageFilter(String pageFilter) {
		PaginationInterceptor.pageFilter = pageFilter;
	}
